...
public class ItemListComponent extends Component {
    ...
    public ItemListComponent(ItemListSetting setting, Button parent, float scaleFactor) {
    ...
    public void draw(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        drawItemPanel(graphics, poseStack, matrix, startY + SEARCH_HEIGHT * scaleFactor + PADDING * scaleFactor, mouseX, mouseY, deltaTime);
    }
    ...
    private void drawItemPanel(GuiGraphics graphics, PoseStack poseStack, Matrix4f matrix, float startY, int mouseX, int mouseY, float deltaTime) {
        ...
        int rows = (int) Math.ceil((double) filteredItems.size() / columns);
        ...
        float itemX = panelX - 3 + PADDING * scaleFactor;
        float itemY = startY + PADDING * scaleFactor - scrollOffset;
        ...
        for (Item item : filteredItems) {
            ...
            float ix = itemX + col * (iconSize + ITEM_PADDING * scaleFactor);
            float iy = itemY + row * (iconSize + ITEM_PADDING * scaleFactor);

            if (iy + iconSize < startY || iy > startY + panelHeight) {
                index++;
                continue;
            }

            boolean selected = itemListSetting.isItemSelected(item);
            boolean hovered = mouseX >= ix && mouseX <= ix + iconSize &&
                    mouseY >= iy && mouseY <= iy + iconSize;
            ...
            DrawHelper.rectangle(poseStack, ix, iy, iconSize, iconSize,
                    ROUNDING * scaleFactor * 0.5f, bgColor.getRGB());

            ResourceLocation texture = itemListSetting.getItemTexture(item);
            if (texture != null) {
                float texturePadding = 1 * scaleFactor;
                DrawHelper.drawTexture(texture, matrix,
                        ix + texturePadding, iy + texturePadding,
    ...
    private void drawScrollbar(PoseStack poseStack, float panelX, float panelY, float panelWidth, float panelHeight, float contentHeight, float maxScroll) {
        ...
        float scrollbarX = panelX + panelWidth - scrollbarWidth;
    ...
    private List<Item> getFilteredItems() {
        ...
        for (Item item : itemListSetting.getAvailableItems()) {
        ...
        filtered.sort(new Comparator<Item>() {
            ...
            public int compare(Item i1, Item i2) {
                boolean selected1 = itemListSetting.isItemSelected(i1);
                boolean selected2 = itemListSetting.isItemSelected(i2);

                if (selected1 && !selected2) return -1;
                if (!selected1 && selected2) return 1;

                ResourceLocation rl1 = BuiltInRegistries.ITEM.getKey(i1);
                ResourceLocation rl2 = BuiltInRegistries.ITEM.getKey(i2);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        float itemX = panelX + PADDING * scaleFactor;
        float itemY = panelY + PADDING * scaleFactor - scrollOffset;
        ...
        for (Item item : filteredItems) {
            ...
            if (mouseX >= ix && mouseX <= ix + iconSize &&
                    mouseY >= iy && mouseY <= iy + iconSize) {
                itemListSetting.toggleItem(item);
                return;
            }
