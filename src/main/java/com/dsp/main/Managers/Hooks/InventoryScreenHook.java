...
public class InventoryScreenHook extends EffectRenderingInventoryScreen<InventoryMenu> implements RecipeUpdateListener {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        float animationDuration = 4.0f; // 0.5 секунды при 20 тиках/сек
        float progress;
        if (isClosing) {
            float time = this.closingTicks + partialTick;
            float rawProgress = Math.min(time / animationDuration, 1.0f);
            progress = 1.0f - easeInOutCubic(rawProgress); // Обратная анимация
        } else {
        ...
        int alphaBg = (int)(progress * 128); // Макс. альфа 128 для полупрозрачности
        int colorBg = (alphaBg << 24) | 0x000000;
        guiGraphics.fill(0, 0, this.width, this.height, colorBg);

        // Трансформация рендера инвентаря (увеличение/уменьшение от центра)
        ...
        // Рендерим книгу рецептов и подсказки (без анимации)
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // Трансформируем координаты мыши для соответствия масштабированному инвентарю
