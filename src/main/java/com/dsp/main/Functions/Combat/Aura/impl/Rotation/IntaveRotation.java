...
public class IntaveRotation extends RotationAngle {
    ...
    public void update(Aura aura, Entity target) {
        ...
        if (randomTargetPos == null || currentTargetPos.distanceTo(randomTargetPos) < THRESHOLD) {
            ...
            currentInterpolation = IntaveRotation.InterpolationType.values()[(int) (Math.random() * IntaveRotation.InterpolationType.values().length)];
        }
    ...
    private Vec3 interpolate(Vec3 from, Vec3 to, float t, IntaveRotation.InterpolationType type) {
        switch (type) {
            case LINEAR:
