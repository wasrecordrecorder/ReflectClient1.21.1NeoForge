...
public class FontRenderer implements Closeable {
    ...
    public void drawString(PoseStack stack, String s, float x, float y, float r, float g, float b, float a) {
        drawString(stack, s, x, y + offset(), r, g, b, a, false);
    }
    ...
    public void drawString(PoseStack stack, String s, float x, float y, float r, float g, float b, float a, boolean gradient) {
        ...
        synchronized (GLYPH_PAGE_CACHE) {
            ...
            // Рендеринг всех текстур одним проходом
            for (ResourceLocation identifier : GLYPH_PAGE_CACHE.keySet()) {
                ...
                for (DrawEntry object : objects) {
                    ...
                    bb.addVertex(mat, xo, yo + h, 0).setUv(u1, v2).setColor(cr, cg, cb, a);
                    bb.addVertex(mat, xo + w, yo + h, 0).setUv(u2, v2).setColor(cr, cg, cb, a);
                    bb.addVertex(mat, xo + w, yo, 0).setUv(u2, v1).setColor(cr, cg, cb, a);
                    bb.addVertex(mat, xo, yo, 0).setUv(u1, v1).setColor(cr, cg, cb, a);
                }
            }

            // Завершаем рендеринг один раз после добавления всех вершин
    ...
    public static @NotNull ResourceLocation randomResourceLocation() {
        return ResourceLocation.fromNamespaceAndPath("minecraft", "dsp/" + randomString(32));
    }
